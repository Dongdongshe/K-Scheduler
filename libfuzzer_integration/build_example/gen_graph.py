...
def inline_counter_table(filename):
    ...
    data = [ele for ele in line.split(', i8*') if ' i8]* @__sancov_gen_' in ele]
    ...
    for ele in data:
        ...
        if bin_text[idx+1:].find('@'+ele.split('@')[1].split(',')[0]+',') != -1:
            new_data.append(ele)
    data = new_data
    ...
    for ele in data:
        ans[ele.split(' ')[9][:-1]] = int(ele.split(' ')[3][2:])
        ordered_key.append(ele.split(' ')[9][:-1])
    ...
    for line in lines:
        if line.startswith('\t'):
            if '[' in line:
                ...
                loc = code.find(' i8]*\l... @__sancov_gen_')
                # convert dot node id to llvm node id
                if loc != -1:
                    tokens = code[loc:].split(')')[0].split(',')
                    local_table, local_edge = tokens[0][tokens[0].find('@'):], tokens[2].split(' ')[2]
                    global_edge = int(local_edge) + inline_table[local_table]
                        ...
                        if ('call' in inst or 'invoke' in inst) and '@' in inst:
                            ...
                            if fun_name in fun_list:
                                ...
                                if global_edge not in node_2_callee:
                                    node_2_callee[dot_id_2_llvm_id[node_id]] = [fun_name]
                                else:
                                    node_2_callee[dot_id_2_llvm_id[node_id]].append(fun_name)
                ...
                #if src_node not in graph: ipdb.set_trace()
...
# This script takes two arguments. The first argument is path to whole program bit code of program whose file names has been truncated (i.e., output file of fix_long_fun_name.py). The second argument is the path to directory of intra-CFGs.
if __name__ == '__main__':
    ...
    with open("katz_cent", "w") as f:
        for key in sorted(scaled_rank.keys()):
            f.write(str(key) + " " + str(round(scaled_rank[key], 14)) + "\n")
    ...
    with open("child_node", "w") as f:
        for key in sorted(global_graph.keys()):
            tmp = ' '.join([str(key)] + [str(ele) for ele in global_graph[key]]) + '\n'
            f.write(tmp)
