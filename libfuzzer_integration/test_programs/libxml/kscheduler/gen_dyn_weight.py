...
if __name__ == '__main__':
    ...
    while True:
        time.sleep(5)
        signal = None
        if not os.path.exists("signal"): continue
        with open("signal", "r") as f: signal = f.read()
        if signal == "1\n":
            t0 = time.time()
            covered_node.clear()
            with open("cur_coverage", "r") as f:
                covered_node = [int(ele) for ele in f.read().split()]
            ...
            for ele in k.ranking():
                ...
                '''
                if ele[1] == min_score:
                    scaled_rank[real_id] = 0
                elif ele[1] == max_score:
                    scaled_rank[real_id] = 10
                else:
                    scaled_rank[real_id] = 10 * (ele[1]-min_score)/(max_score-min_score)
